...
func newHTTPClient(ip string, port int, headerTimeout time.Duration,
	cert tls.Certificate) *http.Client {
	dial := func(network, address string) (net.Conn, error) {
		d := net.Dialer{
			Timeout:   headerTimeout,
			KeepAlive: headerTimeout * 10,
		}
...
func (c *HttpClusterClient) updateClientAddr() {
	...
	for i := range ips {
		if !c.exist(ips[i]) {
			c.add(ips[i], newHTTPClient(ips[i], port, c.cfg.HeaderTimeoutPerRequest, c.cfg.Cert))
		}
	}
